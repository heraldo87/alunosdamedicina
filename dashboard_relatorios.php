...
// Correções:
//  - Filtros de data comparam STRINGS YYYY-MM-DD (ordem lexicográfica = cronológica)
//  - Tabela sempre renderiza; mensagem quando vazia
//  - Botões Aplicar/Limpar chamam recompute corretamente
//  - Incluída seção da nuvem de palavras no HTML (evita erro de innerHTML em elemento nulo)
//  - Mantidas suas credenciais de banco
// ==============================================
  ...
  <div class="max-w-7xl mx-auto">
    ...
    <section class="mb-8 bg-white rounded-xl shadow-lg p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        ...
        <div>
          <label class="block text-sm text-gray-600 mb-1">Colaborador (e-mail)</label>
    ...
    <!-- Qualidade e Relevância (nuvem de palavras) -->
  <!--  <section class="mb-10">
      <div class="bg-white rounded-xl shadow-lg p-6">
        <h3 class="text-xl font-semibold mb-4">Principais Palavras em Atividades Realizadas</h3>
        <div id="word-cloud" class="flex flex-wrap gap-2 justify-center py-4"></div>
      </div>
    </section> -->
        ...
        <table class="min-w-full text-sm">
          <thead>
            <tr class="bg-gray-100 text-gray-700">
              ...
              <th class="px-4 py-2 text-left">ID Colab.</th>
  ...
  <script>
    ...
    function applyFilters(data) {
      ...
      return data.filter(r => {
        ...
        if (coll && String(r.enviar_por_email || '') !== coll) return false;
        return true;
      });
    }
    ...
    function updateSummary(filtered) {
      ...
      const uniqueCollaborators = new Set(filtered.map(i => i.enviar_por_email)).size;
    ...
    function renderCharts(filtered) {
      ...
      // 1) por colaborador
      const aggColab = {};
      filtered.forEach(i => { const k = i.enviar_por_email || '—'; aggColab[k] = (aggColab[k]||0)+1; });
      const labelsColab = Object.keys(aggColab).map(e => (e||'').split('@')[0] || 'n/d');
    ...
    function renderWordCloud(filtered) {
      const box = document.getElementById('word-cloud');
      if (!box) return;
      const common = ['de','da','do','e','a','o','com','para','em','um','uma','no','na','nos','nas','os','as','dos','das'];
      const txt = filtered.map(i => String(i.atividades_realizadas || '').toLowerCase()).join(' ');
      const words = txt.split(/\s+/).filter(w => w.length > 3 && !common.includes(w));
      const wc = {}; words.forEach(w => wc[w] = (wc[w] || 0) + 1);
      const sorted = Object.keys(wc).sort((a,b)=>wc[b]-wc[a]);
      box.innerHTML='';
      sorted.slice(0, 24).forEach((w, i) => {
        const s=document.createElement('span');
        s.textContent = w;
        s.className='font-bold text-gray-700 mx-1';
        s.style.fontSize = `${12 + (24-i)}px`;
        box.appendChild(s);
      });
    }
    ...
    function renderTable(filtered) {
      ...
      filtered.forEach(r => {
        ...
        tr.appendChild(c(r.enviar_por_email));
        tr.appendChild(c(r.cpf));
        tr.appendChild(c(r.id_colaborador));
        tr.appendChild(c(r.data_inicial));
        tr.appendChild(c(r.data_final));
        tr.appendChild(c(r.atividades_realizadas));
        tr.appendChild(c(r.atividades_previstas));
        tr.appendChild(c(r.pontos_relevantes));
    ...
    document.addEventListener('DOMContentLoaded', () => {
      try {
        ...
        const emails = Array.from(new Set(rows.map(r => r.enviar_por_email).filter(Boolean))).sort();
