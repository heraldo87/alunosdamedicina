...
// Prepara e executa a consulta para buscar TODAS as informações relevantes do usuário
// INCLUINDO o novo campo 'profile_image_path'
$stmt = $conn->prepare("SELECT full_name, email, phone, ru, turma, turno, access_level, status_aluno, created_at, profile_image_path FROM usuarios WHERE id = ?");
...
if ($result->num_rows > 0) {
    ...
    $access_level_map = [1 => 'Aluno', 2 => 'Moderador', 3 => 'Administrador'];
    $user_data['access_level_nome'] = $access_level_map[$user_data['access_level']] ?? 'Desconhecido';

    if ($user_data['status_aluno'] == 1) {
        $user_data['status_texto'] = 'Aluno Ativo';
        $status_class = 'bg-green-100 text-green-800';
    } else {
        $user_data['status_texto'] = 'Inativo';
        $status_class = 'bg-red-100 text-red-800';
    }

    $date = new DateTime($user_data['created_at']);
    $user_data['data_cadastro_formatada'] = $date->format('d/m/Y');

    // Define o caminho da imagem do perfil ou um avatar padrão
    $profile_pic = !empty($user_data['profile_image_path']) ? $user_data['profile_image_path'] : 'assets/img/default-avatar.png';
}
                        ...
                        <div class="lg:col-span-1 flex flex-col items-center text-center border-r-0 lg:border-r lg:pr-8">
                            ...
                            <div class="mt-6 w-full">
                                <form action="php/upload_profile_pic.php" method="post" enctype="multipart/form-data">
                                    ...
                                    <input id="profile_pic_upload" name="profile_pic" type="file" class="hidden" onchange="this.form.submit()">
                                </form>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($user_data['access_level_nome']); ?></p>
                                    </div>
                                </div>
